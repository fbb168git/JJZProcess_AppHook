...
//	private static final String FILTER_PKGNAME = "com.fbb.crackme";
	private static final String FILTER_PKGNAME = "com.zcbl";
...
//		if(FILTER_PKGNAME.contains("FILTER_PKGNAME")){
			...
			XposedHelpers.findAndHookMethod(Application.class, "attach", Context.class, new XC_MethodHook() {
				...
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					...
					for(int i =0;i<methods.length;i++){
						...
						builder.append("method"+i+":"+methods[i].getReturnType().getSimpleName() +" "+ methods[i].getName()+"(");
						Class<?>[] clazz = methods[i].getParameterTypes();
						for(int j=0;j<clazz.length;j++){
							builder.append(clazz[j].getSimpleName());
							builder.append(",");
						}
						builder.append(")");
					...
					XposedHelpers.findAndHookMethod(hookclass, "loadClassBinaryName", String.class, ClassLoader.class, List.class,new XC_MethodHook(){
						@Override
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							XposedBridge.log("loadClassBinaryName");
							hookClassInfo((String)param.args[0], (ClassLoader)param.args[1]);
							super.beforeHookedMethod(param);
						}
					});

					XposedHelpers.findAndHookMethod(hookclass, "defineClass", String.class, ClassLoader.class, Object.class, DexFile.class,List.class,new XC_MethodHook(){
						@Override
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							XposedBridge.log("defineClass");
							hookClassInfo((String)param.args[0], (ClassLoader)param.args[1]);
							super.beforeHookedMethod(param);
						}
					});
...
//		}
	...
	private static void hookClassInfo(String className, ClassLoader classLoader){
		...
		if(className.startsWith("com.qihoo")){
			return;
		}
		if(className.startsWith("com.umeng")){
			return;
		}
		if(className.startsWith("com.qti")){
			return;
		}
		XposedBridge.log("className:"+className+" classLoader:"+classLoader);
		XposedBridge.log("-------------------------------");
		boolean tag= false;
		if(!tag){
			return;
		...
		try{
			...
			for(Method method : allMethods){
				...
				XposedBridge.log("signStr:"+signStr);
				//开始构造Hook的方法信息
